...
/*
** +---------------------------------------------------+
** | Name :		~/admin/general/tools_stats.php
** | Begin :	22/09/2006
** | Last :		03/12/2007
** | User :		Genova
	...
	*/
	public function show_stat_image($img_type, $legend)
	{
		...
		Fsb::$tpl->set_blocks('gd', array(
			...
			'LIST_MONTH_BEGIN' =>	Html::create_list($img_type . '_month_begin', $month_begin, $list_month),
			'LIST_YEAR_BEGIN' =>	Html::create_list($img_type . '_year_begin', $year_begin, $list_year),
			'LIST_MONTH_END' =>		Html::create_list($img_type . '_month_end', $month_end, $list_month),
			'LIST_YEAR_END' =>		Html::create_list($img_type . '_year_end', $year_end, $list_year),
			'CURRENT' =>				$current,
			'IMG_TITLE' =>			$title,
			'TYPE' =>				$img_type,
		));
