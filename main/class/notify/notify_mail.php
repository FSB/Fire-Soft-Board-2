...
class Notify_mail
{
    ...
    */
    ...
    private function SmtpConnect() {
        ...
        while($index < count($hosts) && $connection == false)
        {
            ...
            if($this->smtp->Connect($host, $port, $this->Timeout))
            {
                ...
                if($this->SMTPAuth)
                {
                    if(!$this->smtp->Authenticate(Fsb::$cfg->get('smtp_username'), Fsb::$cfg->get('smtp_password')))
                    {
    ...
    /**
     * FSB-ADDON - Set a template file for the mail
     * @access public
     * @return void
     */
	public function set_file($filename)
	{
		...
		{
			$this->SetError($this->Lang("template") . $filename);
		}
    ...
    /**
     * FSB-ADDON - Set and parse some template vars
     * @access public
     * @return void
     */
	public function set_vars($vars = array())
	{
		...
		if (!isset($vars['FORUM_SIG']))
		{
			$vars['FORUM_SIG'] = Fsb::$cfg->get('forum_sig');
		}
	...
	{
		return Fsb::$session->lang('phpmailer_' . $key);
    }
